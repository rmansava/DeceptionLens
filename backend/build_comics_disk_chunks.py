...
Input:  Local copy of comics (fast reads from SSD)
Output: chunk_XXX.faiss  -> NAS (S:/faiss/disk_retrieval/comics_chunks/)
        chunk_XXX_ids.npy -> local SSD (D:/faiss/disk_retrieval/comics_chunk_ids/)
        path_lookup.json  -> local SSD (same dir as IDs)

Paths stored point to the NAS originals (T:/comics/...).
...
# Source: local copy of comics for fast reading (scans all subfolders)
LOCAL_IMAGES_DIR = r"C:\comics"

# Path remapping: stored paths point to NAS originals
NAS_IMAGES_DIR = r"T:\comics"

# Output: FAISS chunks go to NAS (searched via rolling buffer copy)
NAS_CHUNKS_DIR = r"S:\faiss\disk_retrieval\comics_chunks"
LOCAL_CHUNKS_BUFFER = r"D:\faiss\disk_retrieval\comics_chunks"  # Write here first, then copy to NAS
...
def main():
    ...
    if not os.path.exists(LOCAL_IMAGES_DIR):
        ...
        log(f"Copy comics from NAS to local drive first.")
