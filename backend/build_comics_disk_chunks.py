r"""
Build DISK keypoint chunks for comics - direct to chunks with compact IDs.
...
Input:  NAS comics (T:/archiverelated/comics)
Output: chunk_XXX.faiss  -> NAS (T:/faiss/disk_retrieval/comics_chunks/)
        chunk_XXX_ids.npy -> local SSD (D:/faiss/disk_retrieval/comics_chunk_ids/)
        path_lookup.json  -> local SSD (same dir as IDs)

Paths stored point to the NAS originals (T:/archiverelated/comics/...).
...
NAS_SOURCE_DIR = r"T:\archiverelated\comics"

# Local buffer: copy batches here for fast GPU reads
LOCAL_BUFFER_DIR = r"C:\comics_buffer"

# Path prefix stored in path_lookup (matches NAS source)
NAS_IMAGES_DIR = r"T:\archiverelated\comics"

# Output: FAISS chunks go to NAS
NAS_CHUNKS_DIR = r"U:\faiss\disk_retrieval\comics_chunks"
LOCAL_CHUNKS_BUFFER = r"D:\faiss\disk_retrieval\comics_chunks"

# Output: Compact IDs stay on local SSD
CHUNK_IDS_DIR = r"D:\faiss\disk_retrieval\comics_chunk_ids"
...
# Buffer settings - bounded by copied bytes, not giant top-level buckets
BATCH_SIZE_GB = 10
...
def normalize_to_nas_path(path):
    """Normalize local buffer paths to canonical NAS paths."""
...
class FolderBuffer:
    """Manages pipelined copying of units from NAS to local SSD, sized by GB."""

    def __init__(self, all_folders, root_batch_map=None, manifest_state=None):
...
def find_folders_with_new_images(all_folders, scanned_counts, path_to_id, bad_images=None):
    ...
    for i, folder in enumerate(all_folders):
        ...
        if folder in scanned_counts:
            ...
            if current_count == scanned_counts[folder] and known_count >= current_count:
                skipped += 1
                continue
    ...
    if len(all_folders) > 1000:
        print()
    return remaining, skipped


def collect_source_units(source_dir):
    """Collect smaller source units so staging is bounded by copied bytes."""
    units = []
    root_batch_map = {}
...
def main():
    log("=" * 70)
    log("COMICS DISK CHUNK BUILDER (Pipelined NAS -> Local -> GPU)")
    ...
    log(f"Buffer:          {BATCH_SIZE_GB} GB/batch, {MAX_BUFFERED_BATCHES} batches max "
        f"({BATCH_SIZE_GB * MAX_BUFFERED_BATCHES} GB total)")
    log("=" * 70)
    ...
    if manifest_has_work(manifest_state):
        ...
        # List manageable source units on NAS
        log("Scanning NAS for units...")
        ...
        remaining, skipped = find_folders_with_new_images(
            all_folders, scanned_counts, path_to_id, bad_images=known_bad_images
        )
    ...
    # Start folder buffer (NAS -> local SSD)
    buffer = FolderBuffer(remaining, root_batch_map=root_batch_map, manifest_state=manifest_state)
    buffer.start()
    ...
    while buffer.has_more():
        ...
        for folder, folder_size in batch:
            ...
            # Consume prefetched results for GPU
