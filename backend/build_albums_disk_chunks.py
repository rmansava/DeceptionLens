...
Output: chunk_XXX.faiss  -> NAS (U:/faiss/disk_retrieval/albums_chunks/)
...
NAS_CHUNKS_DIR = r"U:\faiss\disk_retrieval\albums_chunks"
...
BATCH_SIZE_GB = 10          # Copy ~10 GB of units per batch
MAX_BUFFERED_BATCHES = 5    # Keep up to 5 batches on local SSD (~50 GB max)
...
def normalize_to_nas_path(path, local_source_dir=None):
    ...
    # Recover older incorrectly stored paths like T:\archiverelated\albums_buffer\...
...
class FolderBuffer:
    ...
    def _restore_from_manifest(self, manifest_state):
        ...
        def restore_ready(folder, size_bytes):
            if folder in seen:
                return
        ...
        for entry in manifest_state.get('ready_units', []):
            restore_ready(entry.get('folder'), entry.get('size_bytes', 0))
            ...
            while batch_bytes < batch_size_bytes:
                ...
                try:
                    ...
                    if is_root_batch:
                        ...
                        src_root = os.path.join(self.source_dir, os.path.dirname(folder))
...
def find_folders_with_new_images(all_folders, scanned_counts, path_to_id, source_dir, bad_images=None):
    ...
    # Count indexed/quarantined images per top-level folder using canonical NAS paths.
    # Compare case-insensitively so resumes survive source layouts that contain both
    # uppercase and lowercase bucket folders.
    known_per_folder = Counter()
    scanned_counts_ci = {str(key).replace('\\', '/').lower(): value for key, value in scanned_counts.items()}
    ...
    for i, folder in enumerate(all_folders):
        ...
        folder_key = folder.replace('\\', '/').lower()
        if '/._batch_' in folder_key:
            remaining.append(folder)
            continue
        known_count = known_per_folder.get(folder_key, 0)
        scanned_count = scanned_counts.get(folder)
        if scanned_count is None:
            scanned_count = scanned_counts_ci.get(folder_key)

        if scanned_count is not None:
            # Normal resume path: folder was fully scanned before, and our indexed/quarantined
            # count is at least what still exists on disk.
            current_count = count_images_in_source(folder, source_dir)
            if current_count == scanned_count and known_count >= current_count:
                skipped += 1
                continue
        elif known_count > 0:
            # Recovery path for older or partial progress files: if we already know about at
            # least as many images as currently exist in the folder, do not rescan it.
            current_count = count_images_in_source(folder, source_dir)
...
def collect_source_units(source_dir):
    """Collect manageable source units so buffering is bounded by bytes, not bucket size."""
    units = []
    root_batch_map = {}
...
def main():
    ...
    if local_mode:
        ...
        log(
            ...
            f"{MAX_BUFFERED_BATCHES} batches max, {BATCH_SIZE_GB * MAX_BUFFERED_BATCHES} GB total)"
        )
    log("=" * 70)
    ...
    if not local_mode and manifest_has_work(manifest_state):
        ...
        # List manageable source units in selected source
        if not os.path.exists(source_dir):
            log(f"ERROR: Source directory not found: {source_dir}")
            return
        log(f"Scanning source for units: {source_dir}")
        all_folders, root_batch_map = collect_source_units(source_dir)
        log(f"Found {len(all_folders):,} source units")

        # Find units with new/unprocessed images
        log("Checking for units with new images...")
        remaining, skipped = find_folders_with_new_images(
            all_folders, scanned_counts, path_to_id, source_dir, bad_images=known_bad_images
        )
        log(f"Skipped {skipped:,} unchanged units, {len(remaining):,} units to process")
        ...
        for folder, folder_size in batch:
            ...
            for j in range(folder_image_total):
                ...
                if tensor is None:
                    ...
                    known_bad_images.add(normalize_to_nas_path(image_path))
                    if total_failed <= 20:
